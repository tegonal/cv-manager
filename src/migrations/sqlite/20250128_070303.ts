...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`cv_skill_groups_order_idx\` ON \`cv_skill_groups\` (\`_order\`);`)
  await db.run(
  ...
  `)
  await db.run(sql`CREATE INDEX \`cv_other_skills_order_idx\` ON \`cv_other_skills\` (\`_order\`);`)
  await db.run(
  ...
  await db.run(sql`CREATE INDEX \`cv_projects_parent_id_idx\` ON \`cv_projects\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`company_organisation_idx\` ON \`company\` (\`organisation_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`project_organisation_idx\` ON \`project\` (\`organisation_id\`);`)
